...
class CryptoFrogZ(IStrategy):
    ...
    custom_stop = {
        ...
        'decay-time': 180,      #166 # minutes to reach end, I find it works well to match this to the final ROI value - default 1080
        'decay-delay': 0,         # minutes to wait before decay starts
        'decay-start': -0.174, # -0.085,     # starting value: should be the same or smaller than initial stoploss - default -0.30
    ...
    # Dynamic ROI
    droi_params_optimize = False
    droi_trend_type = CategoricalParameter(['rmi', 'ssl', 'candle', 'any'], default='any', space='sell', optimize=droi_params_optimize)
    droi_pullback = CategoricalParameter([True, False], default=True, space='sell', optimize=droi_params_optimize)
    droi_pullback_amount = DecimalParameter(0.005, 0.02, default=0.005, space='sell', optimize=droi_params_optimize)
    droi_pullback_respect_table = CategoricalParameter([True, False], default=False, space='sell', optimize=droi_params_optimize)    
    
    # Custom Stoploss
    cstp_params1_optimize = False
    cstp_params2_optimize = False
    cstp_threshold = DecimalParameter(-0.05, 0, default=-0.03, space='sell', optimize=cstp_params1_optimize)
    cstp_bail_how = CategoricalParameter(['roc', 'time', 'any'], default='roc', space='sell', optimize=cstp_params2_optimize)
    cstp_bail_roc = DecimalParameter(-0.05, -0.01, default=-0.03, space='sell', optimize=cstp_params1_optimize)
    cstp_bail_time = IntParameter(720, 1440, default=720, space='sell', optimize=cstp_params2_optimize)
