...
class Hmar(IStrategy):
  ...
  "subplots": {
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['hma20'] = qtpylib.hma(dataframe['close'], 20)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                # (
                (qtpylib.crossed_below(dataframe['rsi'], 32)) &
                # (dataframe['rsi'] < 33) &
                # (dataframe['cci'] < -90 ) &
                # (dataframe['macdhist'].shift(1) > 0) &
                # (dataframe['adx'] > 29) &
                # (dataframe['ema21'] > dataframe['hma20']) &
                # (dataframe['close'].shift(1) < dataframe['hma20']) &
                # (dataframe['open'] > dataframe['hma20']) &
                # (dataframe['close'] > dataframe['hma20']) 
                (dataframe['h-low'] > 1.005) &
                (qtpylib.crossed_below(dataframe['htsine'].shift(1), 0))
                # (qtpylib.crossed_above(dataframe['close'], dataframe['hma20']))
                # ) 
                # | 
                # (
                # (dataframe['cci'] < -90 ) &
                # # (dataframe['macdhist'] < -2.5) &
                # (dataframe['close'].shift(1) < dataframe['hma20']) &
                # (qtpylib.crossed_above(dataframe['close'], dataframe['hma20'])))  # Signal: RSI crosses above 30
            ),
            'buy'] = 1
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (dataframe['h-low'] <= 1)
                |
                (qtpylib.crossed_below(dataframe['htsine'], 98))
                # (
                # (dataframe['close'].shift(1) > dataframe['hma20']) &
                # (qtpylib.crossed_below(dataframe['close'], dataframe['hma20']))
                # ) 
                # | 
                # (dataframe['close'] == dataframe['hma20'])) 
