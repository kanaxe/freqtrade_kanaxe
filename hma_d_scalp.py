...
class hmads(IStrategy):
    ...
    # # Trailing stop:
    # trailing_stop = False
    # trailing_stop_positive = 0.00485
    # trailing_stop_positive_offset = 0.02683
    ...
    # Trailing stop:
    trailing_stop = True
    trailing_stop_positive = 0.02512
    trailing_stop_positive_offset = 0.03552
    trailing_only_offset_is_reached = True
    ...
    # # Trailing stop:
    # trailing_stop = True
    # trailing_stop_positive = 0.00502
    # trailing_stop_positive_offset = 0.01004
    # trailing_only_offset_is_reached = True
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (((dataframe['fastd'] - dataframe['fastk']) > 0 )&
                ((dataframe['fastd'] - dataframe['fastk']) < 15 )&
                (dataframe['fastk'] < dataframe['fastd'])&
                (dataframe['fastd'] < 14)) |
                # (dataframe['upsignal'] < dataframe['hma19'])
                # (dataframe['fastd'] < dataframe['adx'])&
                # (dataframe['upsignal'] < dataframe['sar'])&
               ( (dataframe['downsignal'] < dataframe['hma19'])&
                (dataframe['ol'] > 1.01153))
                # ((dataframe['adx'] < 21) | (dataframe['fastk'] < 18))
                # (dataframe['mfi'] < 32)&
                # (dataframe['fastd'] < 27)
                # (qtpylib.crossed_above(dataframe['cci'], -95)) 
            ),
            'buy'] = 1
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                # ((dataframe['cci'] > 0)&
                # # (qtpylib.crossed_above(dataframe['cci'], dataframe['fastd']))
                ((dataframe['cci'] > 68) | (dataframe['fastk'] > 90)) &
                # |
                # (qtpylib.crossed_above(dataframe['downsignal'], dataframe['hma19'])) &
                # (qtpylib.crossed_above(dataframe['fastk'], dataframe['adx'])) 
